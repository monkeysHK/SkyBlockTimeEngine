...
let h = {
    ...
    checkLocale: function (str) {
        if (typeof str === "string" && (str === h.LOCALES.utc || h.LOCALES.utc_regex.test(str))) return h.LOCALES.utc;
        else if (typeof str === "string" && (str === h.LOCALES.sbst || h.LOCALES.sbst_regex.test(str))) return h.LOCALES.sbst;
        return false;
    },
    getMsg: function (/* ...arguments */) {
        let m = h.FMTMSG[arguments[0]];
        if (!m) return "<msg." + arguments[0] + ">";
        for (let i = 1; i < arguments.length; i++) m = m.replaceAll("$" + i, arguments[i]);
        return m;
    },
    fmtTime: function (hrs, m, s, s_72th) {
        return h
            .getMsg("formatTime", String(hrs).padStart(2, 0), String(m).padStart(2, 0), String(s).padStart(2, 0), s_72th ? h.getMsg("format72th", s_72th) : "")
            .trim();
    },
